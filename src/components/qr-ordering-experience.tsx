...
export default function QrOrderingExperience({
  ...
  async function switchUnpaidBillToUpi(order: TableOrderRecord) {
    ...
    const billPaymentOrderId =
  billActionOrderId ||
  aggregatedUnpaidOrder?.orderId ||
  latestBillOrder?.orderId ||
  unpaidOrders[0]?.orderId ||
  tableOrders[0]?.orderId ||
  "";

if (!billPaymentOrderId) {
  setNoticeMessage("No active order found for payment. Please refresh and try again.");
  return;
}
